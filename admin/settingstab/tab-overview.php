<div class="wrap easy-overview-area">
    <div class="easy-banner-area easy-border-radius-20 overflow-hidden easy-position-rl">
        <div class="easy-banner-content-area">
            <div class="easy-banner-content">
                <img src="<?php echo plugin_dir_url(__DIR__).'img/easy-logo.png';?>" alt="logo">
                <h2 class="title easy-tColor"><?php echo wp_kses_post( __( 'All <span>addons</span> for Elementor page builder', 'easy-elements' ) ); ?></h2>
                <div class="easy-banner-rating">
                    <div class="easy-rating-content">
                        <h3 class="easy-tColor">5.0</h3>
                        <div class="easy-rating">
                            <div class="rating-icon">
                                <i class="dashicons dashicons-star-filled"></i>
                                <i class="dashicons dashicons-star-filled"></i>
                                <i class="dashicons dashicons-star-filled"></i>
                                <i class="dashicons dashicons-star-filled"></i>
                                <i class="dashicons dashicons-star-filled"></i>
                            </div>
                            <span>502 reviews</span>
                        </div>
                    </div>
                    <a href="#" class="easy-btn easy-decoration-none">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
                            <path d="M17 22H7C6.59 22 6.25 21.66 6.25 21.25C6.25 20.84 6.59 20.5 7 20.5H17C17.41 20.5 17.75 20.84 17.75 21.25C17.75 21.66 17.41 22 17 22Z"/>
                            <path d="M20.3502 5.51906L16.3502 8.37906C15.8202 8.75906 15.0602 8.52906 14.8302 7.91906L12.9402 2.87906C12.6202 2.00906 11.3902 2.00906 11.0702 2.87906L9.17022 7.90906C8.94022 8.52906 8.19022 8.75906 7.66022 8.36906L3.66022 5.50906C2.86022 4.94906 1.80022 5.73906 2.13022 6.66906L6.29022 18.3191C6.43022 18.7191 6.81022 18.9791 7.23022 18.9791H16.7602C17.1802 18.9791 17.5602 18.7091 17.7002 18.3191L21.8602 6.66906C22.2002 5.73906 21.1402 4.94906 20.3502 5.51906ZM14.5002 14.7491H9.50022C9.09022 14.7491 8.75022 14.4091 8.75022 13.9991C8.75022 13.5891 9.09022 13.2491 9.50022 13.2491H14.5002C14.9102 13.2491 15.2502 13.5891 15.2502 13.9991C15.2502 14.4091 14.9102 14.7491 14.5002 14.7491Z" />
                        </svg>
                        Upgrade to pro
                    </a>
                </div>
            </div>
        </div>
        <img src="<?php echo plugin_dir_url(__DIR__).'img/overview/banner-shape-1.svg';?>" class="easy-position-ab easy-top-0 easy-start-0 easy-shape-1" alt="logo">
        <img src="<?php echo plugin_dir_url(__DIR__).'img/overview/banner-shape-2.svg';?>" class="easy-position-ab easy-top-0 easy-start-0  easy-shape-2" alt="logo">
        <img src="<?php echo plugin_dir_url(__DIR__).'img/overview/banner-shape-3.png';?>" class="easy-position-ab easy-top-0 easy-end-0  easy-shape-3" alt="logo">
    </div>
</div>