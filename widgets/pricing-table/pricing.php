...
class Easyel_Pricing_Table_Widget extends \Elementor\Widget_Base {
	...
	public function get_style_depends() {
		$handle = 'eel-pricing-table-style';
		$css_path = plugin_dir_path( __FILE__ ) . 'css/pricing.css';
		...
		if ( ! wp_style_is( $handle, 'registered' ) && file_exists( $css_path ) ) {
			wp_register_style( $handle, plugins_url( 'css/pricing.css', __FILE__ ), [], defined( 'WP_DEBUG' ) && WP_DEBUG ? filemtime( $css_path ) : '1.0.0' );
		}	
		return [ $handle ];
	}
	...
	public function get_title() {
		return __( 'Easy Pricing Table', 'easy-elements' );
	}
