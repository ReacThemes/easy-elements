...
class RSHF_Target_Rules_Fields {
	...
	public static function get_location_selections() {
		...
		$special_pages = array(
			'special-404'    => __( '404 Page', 'easy-elements' ),
			'special-search' => __( 'Search Page', 'easy-elements' ),
			'special-blog'   => __( 'Blog / Posts Page', 'easy-elements' ),
			'special-front'  => __( 'Front Page', 'easy-elements' ),
			'special-date'   => __( 'Date Archive', 'easy-elements' ),
			'special-author' => __( 'Author Archive', 'easy-elements' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'easy-elements' );
		}
		...
		$selection_options = array(
			'basic'         => array(
				...
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'easy-elements' ),
					'basic-singulars' => __( 'All Singulars', 'easy-elements' ),
					'basic-archives'  => __( 'All Archives', 'easy-elements' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'easy-elements' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'easy-elements' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'easy-elements' ),
			),
		);
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'easy-elements' ),
				'value' => array(
					'all'        => __( 'All', 'easy-elements' ),
					'logged-in'  => __( 'Logged In', 'easy-elements' ),
					'logged-out' => __( 'Logged Out', 'easy-elements' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'easy-elements' ),
				'value' => array(),
			),
		);
	...
	public function admin_styles() {
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'easy-elements' ),
			'please_delete' => __( 'Please delete', 'easy-elements' ),
			'more_char'     => __( 'or more characters', 'easy-elements' ),
			'character'     => __( 'character', 'easy-elements' ),
			'loading'       => __( 'Loading more results…', 'easy-elements' ),
			'only_select'   => __( 'You can only select', 'easy-elements' ),
			'item'          => __( 'item', 'easy-elements' ),
			'char_s'        => __( 's', 'easy-elements' ),
			'no_result'     => __( 'No results found', 'easy-elements' ),
			'searching'     => __( 'Searching…', 'easy-elements' ),
			'not_loader'    => __( 'The results could not be loaded.', 'easy-elements' ),
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'easy-elements' );
		$saved_values   = $value;
		$output         = '';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'easy-elements' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'easy-elements' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'easy-elements' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'easy-elements' ) . '</a>';
			$output .= '</div>';
		}
