...
if ( ! class_exists( 'HFE_Addons_Actions' ) ) {
	...
	class HFE_Addons_Actions {
		...
		public function __construct() {
			add_action( 'wp_ajax_EE_HFE_Admin_modal', [ $this, 'EE_HFE_Admin_modal' ] );
		 ...
		 * @since 1.6.0
		 */
		public function EE_HFE_Admin_modal() {
			...
			if ( isset( $_POST['plugin'] ) ) {
				...
				if ( 'plugin' === $type ) {
					...
					if ( ! current_user_can( 'activate_plugins' ) ) {
						wp_send_json_error( esc_html__( 'Plugin activation is disabled for you on this site.', 'easy-elements' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						wp_send_json_success( esc_html__( 'Plugin Activated.', 'easy-elements' ) );
					}
				}
				...
				if ( 'theme' === $type ) {
					...
					if ( ! ( current_user_can( 'switch_themes' ) ) ) {
						wp_send_json_error( esc_html__( 'Theme activation is disabled for you on this site.', 'easy-elements' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						wp_send_json_success( esc_html__( 'Theme Activated.', 'easy-elements' ) );
					}
				}
			}

			if ( 'plugin' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate plugin. Please activate from the Plugins page.', 'easy-elements' ) );
			} elseif ( 'theme' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate theme. Please activate from the Themes page.', 'easy-elements' ) );
			}
		}
